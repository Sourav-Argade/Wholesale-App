...
// ====== Page Routing ======
document.querySelector('.side-nav-menu').addEventListener('click', (e) => {
...
function navigateTo(page) {
    ...
    pageTitle.textContent = titles[page] || 'Wholesale App';
    renderPage(page);
}
...
loginForm.onsubmit = async (e) => {
    ...
    try {
        ...
        if (data.success) {
            ...
            showScreen('app');
            $('navUserName').textContent = data.displayName;
            $('navUserRole').textContent = data.role === 'ADMIN' ? 'Admin' : 'Partner';
...
(async function init() {
    if (authToken) {
        // Try a simple GET to check if token is still valid
        try {
            const res = await fetch(`${API_BASE}/customers`, {
                headers: { 'Authorization': `Bearer ${authToken}` }
            });
            if (res.ok) {
                // Reconstruct user from stored info
                const storedUser = localStorage.getItem('currentUser');
                if (storedUser) {
                    currentUser = JSON.parse(storedUser);
                    showScreen('app');
                    $('navUserName').textContent = currentUser.displayName;
                    $('navUserRole').textContent = currentUser.role === 'ADMIN' ? 'Admin' : 'Partner';
                    navigateTo('dashboard');
                    return;
                }
            }
...
async function renderCustomers() {
    mainContent.innerHTML = `
        <div class="search-bar"><input type="text" id="customerSearch" placeholder="Search customers..." oninput="searchCustomers(this.value)"></div>
...
async function refreshMap() {
    ...
    try {
        ...
        if (!hasLocation.length) {
            document.querySelector('#mapLegend').innerHTML = '<p class="text-muted text-sm">No customers with GPS locations yet.</p>';
            return;
        }
        ...
        // Update legend with route colors
        const legendHtml = [];
        try {
            const routes = await api('/routes');
            routes?.forEach((r, i) => {
        ...
        document.querySelector('#mapLegend').innerHTML = legendHtml.length ? legendHtml.join('') : '';
